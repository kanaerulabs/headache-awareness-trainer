...
test.describe.skip("Gamification Feature", () => {
  ...
  test.describe("First Entry Achievement", () => {
    test('should unlock "First Steps" achievement after logging first entry', async ({
      page,
    }) => {
      ...
      await expect(
        page.locator('[data-testid="celebration-modal"]'),
      ...
      await expect(
        modal.locator("text=Logged your first headache entry"),
      ).toBeVisible();
      ...
      const achievementUnlocked = await page.evaluate(async () => {
        ...
        const achievement =
          useGamificationStore.getState().achievements["first-entry"];
  ...
  test.describe("Streak Achievements", () => {
    test("should display streak counter with 0 days initially", async ({
      page,
    }) => {
    ...
    test("should show streak progress to next milestone", async ({ page }) => {
      ...
      await page.evaluate(async () => {
        const { useLoggingStore } =
          await import("@/interface-adapters/store/loggingStore");
  ...
  test.describe("Achievement Grid", () => {
    test("should display achievement grid with all achievements", async ({
      page,
    }) => {
    ...
    test("should show locked state for unearned achievements", async ({
      page,
    }) => {
    ...
    test("should show unlocked state after earning achievement", async ({
      page,
    }) => {
      ...
      const isFirstEntryUnlocked = await page.evaluate(async () => {
        ...
        return useGamificationStore
          .getState()
          .isAchievementUnlocked("first-entry");
      });

      expect(isFirstEntryUnlocked).toBe(true);
    });

    test("should display achievement categories correctly", async ({
      page,
    }) => {
      ...
      const categories = await page.evaluate(async () => {
        ...
        const streakCount = Object.keys(achievements).filter((id) =>
          id.startsWith("streak-"),
        ).length;
        const firstActionCount = Object.keys(achievements).filter((id) =>
          id.startsWith("first-"),
        ).length;
        const entryMilestoneCount = Object.keys(achievements).filter((id) =>
          id.startsWith("entries-"),
        ).length;
        const checkinMilestoneCount = Object.keys(achievements).filter((id) =>
          id.startsWith("checkins-"),
        ).length;
  ...
  test.describe("Milestone Achievements", () => {
    test("should unlock entries-10 achievement after 10 entries", async ({
      page,
    }) => {
      // Log 10 entries
      for (let i = 0; i < 10; i++) {
      ...
      const isUnlocked = await page.evaluate(async () => {
        ...
        return useGamificationStore
          .getState()
          .isAchievementUnlocked("entries-10");
      });

      expect(isUnlocked).toBe(true);
    });

    test("should unlock checkins-10 achievement after 10 check-ins", async ({
      page,
    }) => {
      // Log 10 check-ins
      for (let i = 0; i < 10; i++) {
      ...
      const isUnlocked = await page.evaluate(async () => {
        ...
        return useGamificationStore
          .getState()
          .isAchievementUnlocked("checkins-10");
      });
  ...
  test.describe("Micro-Win Messages", () => {
    test("should show different messages based on context", async ({
      page,
    }) => {
      ...
      const firstMessage = await page.evaluate(async () => {
        const { useGamificationStore } =
          await import("@/interface-adapters/store/gamificationStore");
        const msg = useGamificationStore
          .getState()
          .getMicroWinMessage({ isFirstEntry: true });
        return msg?.message;
      });
    ...
    test("should show streak messages for consecutive days", async ({
      page,
    }) => {
      ...
      const streakMessage = await page.evaluate(async () => {
        ...
        const msg = useGamificationStore
          .getState()
          .getMicroWinMessage({ currentStreak: 5 });
        return msg?.message;
      });
      ...
      const matchesAny = validMessages.some((msg) =>
        streakMessage?.includes(msg),
      );
  ...
  test.describe("Celebration Modal", () => {
    test("should display celebration modal with correct achievement details", async ({
      page,
    }) => {
    ...
    test("should close celebration modal on continue button click", async ({
      page,
    }) => {
      // Log first entry
      await page.goto("/log");
  ...
  test.describe("IndexedDB Persistence", () => {
    test("should persist achievements across page reloads", async ({
      page,
    }) => {
