...
test.describe("Learn Page", () => {
  ...
  test.describe("Page Load and Content Display", () => {
    ...
    test("should display overall progress bar", async ({ page }) => {
      ...
      // Verify progress percentage is displayed (e.g., "50%")
      ...
      const progressBar = page
        ...
        .or(page.locator('[aria-label*="progress"]'));
    ...
    test('should display "Start Learning" section', async ({ page }) => {
      ...
      await expect(
        page
          .locator("#available-content-heading")
          .or(page.getByText("Start Learning")),
      ).toBeVisible();
    });
  });

  test.describe("Content Navigation", () => {
    test("should navigate to content detail page when card clicked", async ({
      page,
    }) => {
      await page.goto("/learn");
    ...
    test("should display different content IDs for multiple cards", async ({
      page,
    }) => {
      await page.goto("/learn");
    ...
    test("should return to learn page from content detail", async ({
      page,
    }) => {
      await page.goto("/learn");
  ...
  test.describe("Mobile Viewport (375x667 - iPhone SE)", () => {
    ...
    test("should display content cards in single column on mobile", async ({
      page,
    }) => {
      await page.goto("/learn");
    ...
    test("should display touch-friendly content cards on mobile", async ({
      page,
    }) => {
      await page.goto("/learn");
    ...
    test("should not overflow viewport horizontally on mobile", async ({
      page,
    }) => {
      await page.goto("/learn");
  ...
  test.describe("Desktop Viewport (1280x720)", () => {
    ...
    test("should display content cards in multi-column grid on desktop", async ({
      page,
    }) => {
      await page.goto("/learn");
  ...
  test.describe("Progress Tracking", () => {
    ...
    test("should display progress bar visually", async ({ page }) => {
      ...
      const hasValue = await firstProgressBar
        .getAttribute("aria-valuenow")
        .catch(() => null);
      const hasStyle = await firstProgressBar
        .getAttribute("style")
        .catch(() => null);
  ...
  test.describe("Accessibility", () => {
    test("should have accessible page structure", async ({ page }) => {
      ...
      await expect(
        page.locator("#available-content-heading, h2").first(),
      ).toBeVisible();
    });
    ...
    test("should have accessible progress indicator", async ({ page }) => {
      ...
      if (await firstBar.isVisible()) {
        const ariaLabel = await firstBar
  ...
  test.describe("Navigation via Bottom Nav", () => {
    test("should navigate to learn page from home via bottom nav", async ({
      page,
    }) => {
      await page.goto("/");
