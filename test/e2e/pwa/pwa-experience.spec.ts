...
test.describe("PWA Mobile Experience", () => {
  ...
  test.describe("Install Prompt", () => {
    test.skip("should display install prompt after delay", async ({ page }) => {
      ...
      await expect(page.locator("#install-prompt-title")).toContainText(
        "Install Headache Trainer",
      );
      await expect(page.locator("#install-prompt-description")).toBeVisible();
    });

    test.skip("should dismiss install prompt when X button clicked", async ({
      page,
    }) => {
      await page.goto("/");
    ...
    test.skip("should dismiss install prompt on Escape key", async ({
      page,
    }) => {
      await page.goto("/");
    ...
    test.skip("should show iOS instructions on iOS devices", async ({
      page,
      context,
    }) => {
      // Emulate iOS device
      ...
      await expect(page.locator("#install-prompt-description")).toContainText(
        "Add to Home Screen",
      );

      // Verify iOS share icon is visible
      await expect(
        page.locator('[data-testid="ios-share-icon"]'),
      ).toBeVisible();
    });

    test.skip("should show install button on non-iOS devices", async ({
      page,
    }) => {
      await page.goto("/");
  ...
  test.describe("Responsive Layout", () => {
    test.skip("should display mobile-optimized layout on small screens", async ({
      page,
    }) => {
      // Set mobile viewport
    ...
    test("should maintain proper spacing with bottom navigation", async ({
      page,
    }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      await page.goto("/");
  ...
  test.describe("Touch Interactions", () => {
    ...
    test.skip("should show touch feedback on button press", async ({
      page,
    }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      await page.goto("/");
