...
test.describe("Insights & Patterns Page", () => {
  ...
  test.describe("Page Load and Initial State", () => {
    test("should display insights page with correct role and aria-label", async ({
      page,
    }) => {
      ...
      await expect(insightsPage).toHaveAttribute(
        "aria-label",
        "Insights & Patterns",
      );
    });
    ...
    test("should display page header with title and subtitle", async ({
      ...
      await expect(
        page.getByRole("heading", { name: "Insights & Patterns", level: 1 }),
      ).toBeVisible();
      await expect(
        page.getByText(
    ...
    test("should show loading skeleton initially", async ({ page }) => {
      ...
      const loadingAnnouncement = page
        .locator("role=status")
        .filter({ hasText: "Loading insights..." });
    ...
    test("should display all main sections after loading", async ({ page }) => {
      ...
      await expect(
        page.locator('[data-testid="calendar-section"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="correlations-section"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="trends-section"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="time-of-day-section"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="personal-insights-section"]'),
      ).toBeVisible();
      await expect(
        page.locator('[data-testid="general-insights-section"]'),
      ).toBeVisible();
    });
  });
  ...
  test.describe("Correlation Analysis", () => {
    test("should display correlations section", async ({ page }) => {
      ...
      const correlationsSection = page.locator(
    ...
    test("should display correlation heading", async ({ page }) => {
      ...
      await expect(page.locator("#correlations-heading")).toContainText(
    ...
    test("should show empty state when no correlations exist", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
      ...
      const hasEmptyState = await correlationBars
        .getByText(/no correlation|not enough data/i)
        .isVisible()
        .catch(() => false);

      // Either shows correlations or empty state (both are valid)
      const hasBars =
        (await correlationBars.locator('[role="progressbar"]').count()) > 0;
  ...
  test.describe("Trend Charts", () => {
    ...
    test("should display trend heading", async ({ page }) => {
      ...
      await expect(page.locator("#trends-heading")).toContainText(
        "Headaches over time",
      );
    });
    ...
    test("should display trend description", async ({ page }) => {
      ...
      // Verify description
      await expect(
        page.getByText("Track your headache frequency and intensity trends"),
      ).toBeVisible();
    });
    ...
    test("should show empty state when no trend data exists", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
      ...
      const hasEmptyState = await trendsSection
        .getByText(/no trend|not enough data/i)
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Time of Day Analysis", () => {
    test("should display time of day section", async ({ page }) => {
      ...
      // Verify section is visible
      const timeOfDaySection = page.locator(
    ...
    test("should display time of day analysis component", async ({ page }) => {
      ...
      const timeOfDayAnalysis = page.locator(
        '[data-testid="time-of-day-analysis"]',
      );
      await expect(timeOfDayAnalysis).toBeVisible();
    });

    test("should show empty state when no time data exists", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
      ...
      const hasEmptyState = await timeOfDaySection
        .getByText(/no time|not enough data/i)
        .isVisible()
        .catch(() => false);
      const hasAnalysis = await page
        .locator('[data-testid="time-of-day-analysis"]')
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Personal Insights", () => {
    test("should display personal insights section", async ({ page }) => {
      ...
      // Verify section is visible
      const personalInsightsSection = page.locator(
    ...
    test("should display personal insights heading", async ({ page }) => {
      ...
      await expect(page.locator("#personal-insights-heading")).toContainText(
        "Personal Insights",
      );
    });
    ...
    test("should be collapsed by default", async ({ page }) => {
      ...
      const toggleButton = personalInsightsSection.locator(
        'button[aria-controls="personal-insights-content"]',
      );
    ...
    test("should expand when toggle button clicked", async ({ page }) => {
      ...
      const content = personalInsightsSection.locator(
        "#personal-insights-content",
      );
      await expect(content).toBeVisible();
    });

    test("should collapse when toggle button clicked twice", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
    ...
    test("should show locked message for new users", async ({ page }) => {
      ...
      const hasLockedMessage = await personalInsightsSection
        .getByText(/No personal insights available yet|Keep logging check-ins/i)
        .isVisible()
        .catch(() => false);
      const hasInsights =
        (await personalInsightsSection
  ...
  test.describe("General Insights", () => {
    test("should display general insights section", async ({ page }) => {
      ...
      // Verify section is visible
      const generalInsightsSection = page.locator(
    ...
    test("should display general insights heading", async ({ page }) => {
      ...
      await expect(page.locator("#general-insights-heading")).toContainText(
        "General Insights",
      );
    });
    ...
    test("should display description", async ({ page }) => {
      ...
      // Verify description
      await expect(
        page.getByText("Research-backed headache insights and tips"),
      ).toBeVisible();
    });
    ...
    test("should be expanded by default", async ({ page }) => {
      ...
      const toggleButton = generalInsightsSection.locator(
        'button[aria-controls="general-insights-content"]',
      );

      await expect(toggleButton).toHaveAttribute("aria-expanded", "true");

      // Verify content is visible
      const content = generalInsightsSection.locator(
    ...
    test("should display insight cards", async ({ page }) => {
      ...
      const insightCards = generalInsightsSection.locator(
  ...
  test.describe("Navigation", () => {
    test("should navigate from dashboard to insights via bottom nav", async ({
      page,
    }) => {
      // Start at dashboard
    ...
    test("should maintain filter state after navigating away and back", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
      ...
      // Verify 90 days is selected
      await expect(page.locator('[data-testid="filter-90"]')).toHaveAttribute(
        "aria-selected",
        "true",
      );
  ...
  test.describe("Accessibility", () => {
    ...
    test("should support keyboard activation for toggle buttons", async ({
      page,
    }) => {
      // Wait for page to load
      await page.waitForTimeout(2000);
...
test.describe("Insights Page - Responsive Design", () => {
  test.describe("Mobile Viewport (375x667)", () => {
    ...
    test("should make collapsible sections work on mobile", async ({
      page,
    }) => {
      await page.goto("/insights");
      await page.waitForTimeout(2000);
  ...
  test.describe("Desktop Viewport (1280x720)", () => {
    ...
    test("should have adequate spacing between sections on desktop", async ({
      page,
    }) => {
      await page.goto("/insights");
      await page.waitForTimeout(2000);
      ...
      expect(calendarBox!.y + calendarBox!.height).toBeLessThan(
        correlationsBox!.y,
      );
    });
  });
