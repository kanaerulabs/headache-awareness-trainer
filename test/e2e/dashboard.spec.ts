...
test.describe('Dashboard Page', () => {
  ...
  test.describe('Page Load and Initial State', () => {
    test('should display dashboard page with correct role and aria-label', async ({ page }) => {
    ...
    test('should display page header with title and subtitle', async ({ page }) => {
      ...
      await expect(page.getByText('Your headache awareness journey')).toBeVisible();
    });
    ...
    test('should show loading skeleton initially', async ({ page }) => {
      ...
      const loadingAnnouncement = page.locator('role=status').filter({ hasText: 'Loading dashboard...' });
    ...
    test('should display all 6 main components after loading', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);

      // Verify all dashboard components are present
      await expect(page.locator('[data-testid="quick-insight-card"]')).toBeVisible();
      ...
      await expect(page.locator('[data-testid="quick-action-buttons"]')).toBeVisible();
      await expect(page.locator('[data-testid="recent-entries-list"]')).toBeVisible();
    });
  });

  test.describe('Data Display - No Data State', () => {
    test('should display welcome insight when no data exists', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);
    ...
    test('should display recent entries section (may be empty)', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);

      const recentEntriesList = page.locator('[data-testid="recent-entries-list"]');
  ...
  test.describe('Navigation - Quick Action Buttons', () => {
    test('should navigate to /log when "Log Headache" is clicked', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);
    ...
    test('should navigate to /checkin when "Quick Check-in" is clicked', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);

      // Find and click "Quick Check-in" button
      const checkinButton = page.locator('[data-testid="quick-action-buttons"]').getByRole('button', { name: /check-in|check in/i });
    ...
    test('should have Quick Actions section with proper ARIA label', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);

      // Verify Quick Actions section has proper landmark
      const quickActionsSection = page.locator('section[aria-labelledby="quick-actions-heading"]');
  ...
  test.describe('Data Display - With Data', () => {
    test.beforeEach(async ({ page }) => {
      ...
      const intensity4 = page.locator('[data-testid="intensity-section"] button').filter({ hasText: '4' });
    ...
    test('should display recent entries after data is created', async ({ page }) => {
      const recentEntriesList = page.locator('[data-testid="recent-entries-list"]');
      await expect(recentEntriesList).toBeVisible();
    ...
    test('should update weekly summary counts after data is created', async ({ page }) => {
      const weeklySummary = page.locator('[data-testid="weekly-summary-card"]');
      await expect(weeklySummary).toBeVisible();
    ...
    test('should display contextual insight based on data', async ({ page }) => {
      const insightCard = page.locator('[data-testid="quick-insight-card"]');
      await expect(insightCard).toBeVisible();
  ...
  test.describe('Recent Activity Section', () => {
    test('should have Recent Activity section with proper ARIA label', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);

      // Verify Recent Activity section has proper landmark
      const recentActivitySection = page.locator('section[aria-labelledby="recent-activity-heading"]');
    ...
    test('should display recent entries with timestamps', async ({ page }) => {
      ...
      const hasEntries = await recentEntriesList.locator('[data-testid^="entry-"]').count() > 0;
  ...
  test.describe('Refresh Functionality', () => {
    test('should refresh dashboard data when insight card refresh is clicked', async ({ page }) => {
      // Wait for loading to complete
      await page.waitForTimeout(2000);
      ...
      const refreshButton = insightCard.locator('button[aria-label*="refresh" i], button[aria-label*="reload" i]');
...
test.describe('Responsive Layout - Mobile Viewport', () => {
  ...
  test('should allow scrolling to view all dashboard components on mobile', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);

    // Scroll to recent entries (bottom of page)
    await page.locator('[data-testid="recent-entries-list"]').scrollIntoViewIfNeeded();
    await expect(page.locator('[data-testid="recent-entries-list"]')).toBeVisible();

    // Scroll back to top
    await page.locator('[data-testid="quick-insight-card"]').scrollIntoViewIfNeeded();
    await expect(page.locator('[data-testid="quick-insight-card"]')).toBeVisible();
  });

  test('should have touch-friendly quick action buttons on mobile', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);

    const quickActionButtons = page.locator('[data-testid="quick-action-buttons"]');
  ...
  test('should navigate correctly on mobile taps', async ({ page }) => {
    ...
    const logButton = page.locator('[data-testid="quick-action-buttons"]').getByRole('button').first();
    await logButton.click();
...
test.describe('Responsive Layout - Tablet Viewport', () => {
  ...
  test('should navigate correctly on tablet', async ({ page }) => {
    ...
    const checkinButton = page.locator('[data-testid="quick-action-buttons"]').getByRole('button').last();
    await checkinButton.click();
...
test.describe('Responsive Layout - Desktop Viewport', () => {
  ...
  test('should display stats in grid layout on desktop', async ({ page }) => {
    ...
    const streakBox = await page.locator('[data-testid="streak-display"]').boundingBox();
    const trendBox = await page.locator('[data-testid="trend-indicator"]').boundingBox();

    expect(streakBox).not.toBeNull();
    expect(trendBox).not.toBeNull();
  });

  test('should display all components without scrolling on desktop', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);

    // Verify all main components are in viewport
    await expect(page.locator('[data-testid="quick-insight-card"]')).toBeInViewport();
    await expect(page.locator('[data-testid="streak-display"]')).toBeInViewport();
    await expect(page.locator('[data-testid="weekly-summary-card"]')).toBeInViewport();
    await expect(page.locator('[data-testid="quick-action-buttons"]')).toBeInViewport();
  });
});
...
test.describe('Accessibility - Tab Navigation', () => {
  ...
  test('should activate quick action button with Enter key', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);
  ...
  test('should activate quick action button with Space key', async ({ page }) => {
    await page.goto('/dashboard');
    await page.waitForTimeout(2000);
  ...
  test('should show focus indicators on interactive elements', async ({ page }) => {
    ...
    // Verify focus moved
    const isFocused = await checkinButton.evaluate((el) => el === document.activeElement);
    expect(isFocused || await logButton.evaluate((el) => el === document.activeElement)).toBe(true);
  });
});
...
test.describe('Accessibility - Screen Reader Support', () => {
  test('should have proper ARIA landmarks', async ({ page }) => {
    ...
    await expect(page.locator('section[aria-labelledby="quick-actions-heading"]')).toBeVisible();
    await expect(page.locator('section[aria-labelledby="recent-activity-heading"]')).toBeVisible();
  });
  ...
  test('should announce loading state to screen readers', async ({ page }) => {
    ...
    const loadingAnnouncement = page.locator('[role="status"][aria-live="polite"]').filter({ hasText: /loading/i });

    // Loading may be very brief, so we check if it existed or if page loaded directly
    const wasLoadingAnnounced = await loadingAnnouncement.count() > 0 ||
...
test.describe('Data Persistence', () => {
  test('should persist dashboard data across page reloads', async ({ page }) => {
    // Create test data
    await page.goto('/checkin');
    await page.waitForTimeout(1000);
    ...
    const weeklySummaryBefore = await page.locator('[data-testid="weekly-summary-card"]').textContent();
    ...
    const weeklySummaryAfter = await page.locator('[data-testid="weekly-summary-card"]').textContent();
